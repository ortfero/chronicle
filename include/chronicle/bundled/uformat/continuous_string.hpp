...
namespace uformat {
    ...
    public:
        ...
        continuous_string(continuous_string&& other) noexcept
            : reserved_capacity_ {other.reserved_capacity_},
              committed_capacity_ {other.committed_capacity_},
              size_ {other.size_},
              data_ {other.data_} {
        ...
        bool push_back(char c) noexcept {
            if(size_ + 1 == committed_capacity_
               && !commit(committed_capacity_ + 1))
                return false;
            data_[size_] = c;
            ++size_;
            return true;
    ...
    private:
        ...
        static char* reserve_pages(size_type capacity) noexcept {
            ...
            return static_cast<char*>(
                VirtualAlloc(nullptr, capacity, MEM_RESERVE, PAGE_NOACCESS));
#endif
        }
        ...
        bool reserve() noexcept {
            ...
            if(MCAP % page_size != 0 || pages_count == 0)
                ++pages_count;
            auto const reserved_capacity =
                nearest_power_of_2(pages_count * page_size);
        ...
        bool commit(size_type new_capacity) noexcept {
            ...
            auto committed =
                commit_pages(&data_[size_], new_capacity - committed_capacity_);
            if(committed == nullptr)
                return false;
    ...
    std::string& operator+=(std::string const& x,
                            continuous_string<N> const& y) {
        return x.append(y.data(), y.size());
    }


    template<std::size_t N>
    bool operator==(continuous_string<N> const& x,
                    continuous_string<N> const& y) {
